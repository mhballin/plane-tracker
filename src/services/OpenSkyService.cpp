...
int OpenSkyService::fetchAircraft(Aircraft* aircraftList, int maxAircraft) {
    ...
    // Rate limiting — return immediately if called too soon; scheduler will retry on next interval
            ...
            if (!error) {
                ...
                if (!states.isNull()) {
                    for (JsonArray state : states) {
                        ...
                        if (plane.onGround) { plane.valid = false; continue; }
                        ...
                        if (plane.airline == "Private") { plane.valid = false; continue; }
                        aircraftCount++;
                    }
